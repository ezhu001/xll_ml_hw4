...
AddIn xai_option_cgf(
	...
	.Arguments({
		...
		.Category(CATEGORY)
	.FunctionHelp(L"Return cumulant generating function (CGF) value of an option model.")
);
...
// I feel like this is already TODO: implement OPTION.BLACK.PUT, I will try the CALL instead I guess
...
// TODO: implement OPTION.BLACK.PUT (I will try CALL, as I see PUT is already up there)
...
// TODO: implement OPTION.BSM.PUT (also I will try with CALL after PUT)

AddIn xai_option_bsm_put(
	Function(XLL_DOUBLE, L"xll_option_bsm_put", CATEGORY L".BSM.PUT")
	.Arguments({
		Arg(XLL_DOUBLE, L"r", L"is the risk-free interest rate."),
...
AddIn xai_option_bsm_call(
	...
	.Arguments({
		...
		.Category(CATEGORY)
	.FunctionHelp(L"Return price of a European call option under the Black-Scholes-Merton model.")
);
double WINAPI xll_option_bsm_call(double r, double s0, double sigma, double k, double t, HANDLEX m)
{
#pragma XLLEXPORT
	double result = NaN<double>;
	...
	try {
		...
		result = black::bsm::call(r, s0, sigma, k, t, *m_);
	}
	catch (const std::exception& ex) {
		XLL_ERROR(ex.what());
	}
